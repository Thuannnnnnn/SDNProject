...
export const uploadUserAvatar = async (req, res) => {
  ...
  upload(req, res, async function (err) {
    ...
    try {
      ...
      const exists = await blockBlobClient.exists();
      if (exists) {
        return res.status(400).json({ message: 'Image already exists. Use update endpoint to change the avatar.' });
      }
      ...
      const fileUrl = blockBlobClient.url;
      return res.status(200).json({ message: 'Image uploaded successfully', fileUrl });
...
export const updateUserAvatar = async (req, res) => {
  ...
  upload(req, res, async function (err) {
    ...
    try {
      ...
      const exists = await blockBlobClient.exists();
      if (!exists) {
        return res.status(404).json({ message: 'No existing image found. Use upload endpoint to add a new avatar.' });
      }
      ...
      const fileUrl = blockBlobClient.url;
      return res.status(200).json({ message: 'Image updated successfully', fileUrl });
